...
- (void)layoutSubviews
{
    ...
    _trackImageViewNormal.frame = CGRectMake(0, self.bounds.size.height/2, self.bounds.size.width, self.bounds.size.height/2);
    _trackImageViewHighlighted.frame =  CGRectMake(0, self.bounds.size.height/2, self.bounds.size.width, self.bounds.size.height/2);

    
    // the thumb
    CGFloat thumbHeight = self.bounds.size.height;//98.f *  _trackImageViewNormal.bounds.size.height / 64.f;   // thumb height is relative to track height
    CGFloat thumbWidth = 91.f * 2* thumbHeight / 98.f; // thumb width and height keep the same ratio as the original image size
...
- (void)commonInit
{
    ...
    _labelOnThumb.text = [NSString stringWithFormat:[self valueStringFormat], _value];
    _labelOnThumb.textColor = [UIColor blackColor];
